...
array_pic_num_mnist_all = np.load(str(your_path) + '/FI_Image_Choose/adv_info/sample_info/Mnist_sheet(%d)_situation(1)_FI_below(0.01)_pro_y_target(0.00)_array.npy'%(sheet_id))
...
for i_pic in range(cut_start,cut_end,1):  #  [0:correct_class, 1:target_class, 2:pic_num]
    ...
    np.save(str(your_path) + '/FI_Image_Choose/adv_info/adv_Mnist/train/x/adv(%d)_mnist(%d)_sheet(%d)_situation(1)_FI_below(0.01)_pro_y_target(0.20)_POS_critical_point_adv_array.npy' % (i_pic,pic_num_i, sheet_id),x_adv_of_i)
    np.save(str(your_path) + '/FI_Image_Choose/adv_info/adv_Mnist/train/y/adv(%d)_mnist(%d)_sheet(%d)_situation(1)_FI_below(0.01)_pro_y_target(0.20)_POS_critical_point_adv_array.npy' % (i_pic,pic_num_i, sheet_id),y_adv_of_i)
