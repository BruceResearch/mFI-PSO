...
def calculate_object_score(m,posi_m,target_class,correct_class,x_pop,x_adv,p_erro,a1=1000000,a2=1):  
    ...
    for i in range(pop_size):
        ...
        y_pre = resnet32.predict(pixel_adv.reshape((1,32,32,3))*255)  

        k_sort = np.argsort(y_pre)  
...
def PSO_critical_point(pic_num,pixel_FI_array, m ,correct_class,target_class ,road_str_adv,sheet_id =2 ):
    ...
    if sheet_id == 3:
        ...
        y_test = np.load(str(your_path) + '/FI_Image_Choose/Cifar_test_label.npy')
    ...
    posi_m = posi_m_func(pixel_FI_array,m)        # print('position_of_m:',posi_m)
    ...
    for j in range(3072):
        ...
        x_adv_final_info_list.append(x_adv_final[0][j])  

    
    y_adv_pre = resnet32.predict(x_adv_final.reshape((1,32,32,3))*255)
    print('probability after adding the perturbation：',y_adv_pre)
